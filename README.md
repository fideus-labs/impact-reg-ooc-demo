# impact-reg out-of-core registration demo

Scripts, presets, figures and the talk of the APEX / BRAIN CONNECTS hackathon demo (UT Austin, September 2026): patch-based
registration with [KonfAI](https://github.com/fideus-labs/KonfAI) and [impact-reg-konfai](https://pypi.org/project/impact-reg-konfai/),
driving elastix, FireANTs and ConvexAdam, with the IMPACT losses on segmentation-model feature maps, on volumes that do not fit in memory.

**What is here:** the code (`*.py`, `scripts/`, `apex/`, `topic4/`), the presets (`presets/`), the figures the text refers to
(`figures/`, screenshots only), the talk (`talk/`), the slide deck (`deck/`) and the competition results (`docs/apex_results.md`).
